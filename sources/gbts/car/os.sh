...
GBT__OS_SYMBOLS=(
    [amzn]='\xef\x89\xb0'                [amzn_color]=208
    ...
    [cloud]='\xef\x83\x82'               [cloud_color]=39
    [coreos]='\xef\x8c\x85'              [coreos_color]=32
    [darwin]='\xef\x8c\x82'              [darwin_color]=15
    [debian]='\xef\x8c\x86'              [debian_color]=88
    [devuan]='\xef\x8c\x87'              [devuan_color]=16
    [docker]='\xef\x8c\x88'              [docker_color]=26
    ...
    [manjaro-arm]='\xef\x8c\x92'         [manjaro-arm_color]=34
...
function GbtCarOs() {
    ...
    if [ -n "$GBT_CAR_OS_NAME" ]; then
        os=$GBT_CAR_OS_NAME
    elif [ -e /proc/1/sched ] && ! head -n1 /proc/1/sched | grep -Eq '(init|systemd)'; then
        os='docker'
    else
        os="$(uname -s)"
